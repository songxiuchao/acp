...
object MoneyToCN {
     ...
     */
    private val CN_UPPER_RENMINBI = arrayOf("分", "角", "元", "拾", "佰", "仟", "万", "拾", "佰", "仟", "亿", "拾", "佰", "仟", "兆", "拾", "佰", "仟")
     ...
     */
    @JvmStatic
    fun moneyToRenminbi(money: Double): String {
        ...
        var number = numberOfMoney.movePointRight(MONEY_PRECISION)
                .setScale(0, DecimalProcessModeEnum.HalfUp.mode).abs().toLong()
        // 得到小数点后两位值
        ...
        while (number > 0) {
            ...
            if (numUnit > 0) {
                if (numIndex == 9 && zeroSize >= 3) {
                    sb.insert(0, CN_UPPER_RENMINBI[6])
                }
                if (numIndex == 13 && zeroSize >= 3) {
                    sb.insert(0, CN_UPPER_RENMINBI[10])
                }
                sb.insert(0, CN_UPPER_RENMINBI[numIndex])
