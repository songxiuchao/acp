...
public class CustomerRedisTokenStore implements TokenStore {
        ...
        try {
            ...
            if (refreshToken != null && refreshToken.getValue() != null) {
                ...
                if (refreshToken instanceof ExpiringOAuth2RefreshToken) {
                    expirationToken((ExpiringOAuth2RefreshToken) refreshToken, conn, refreshToAccessKey, accessToRefreshKey);
                }
            }
            conn.closePipeline();
        } finally {
    ...
    private void expirationToken(ExpiringOAuth2RefreshToken expiringRefreshToken, RedisConnection conn, byte[] refreshTokenKey, byte[] accessTokenKey) {
        Date expiration = expiringRefreshToken.getExpiration();
        if (expiration != null) {
            long seconds = (expiration.getTime() - System.currentTimeMillis()) / 1000L;
            conn.expire(refreshTokenKey, seconds);
            conn.expire(accessTokenKey, seconds);
        }
    }
    ...
    public void storeRefreshToken(OAuth2RefreshToken refreshToken, OAuth2Authentication authentication) {
        ...
        try {
            ...
            if (refreshToken instanceof ExpiringOAuth2RefreshToken) {
                expirationToken((ExpiringOAuth2RefreshToken) refreshToken, conn, refreshKey, refreshAuthKey);
            }
            conn.closePipeline();
        } finally {
            conn.close();
